...
public class P_Priority {
	Process[] pArr;
	public P_Priority(Process[] pArr) {
		this.pArr = pArr;
	}
	...
	public void Play() {
		...
		int[] prio = new int[pArr.length + 1];

		for(int i = 0; i < pArr.length; i++) {
			rt[i] = pArr[i].getbT();
			prio[i] = pArr[i].getPriority();
		}
		rt[pArr.length] = 1000;
		prio[pArr.length] = 1000;

		for(time = 0; done != pArr.length; time++) {
			smallest = pArr.length;
			for(int i = 0; i < pArr.length; i++) {
				if(pArr[i].getaT() <= time && prio[i] < prio[smallest] && rt[i]>0)
					smallest = i;
			}
			rt[smallest]--;
